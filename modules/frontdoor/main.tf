...
resource "azurerm_cdn_frontdoor_origin" "app2" {
  ...
  host_name          = var.app2_default_hostname
  http_port          = 80
  https_port         = 443
  origin_host_header = var.app2_default_hostname
  priority           = 1
  weight             = 1000
  ...
  private_link {
    ...
    private_link_target_id = var.app2_id
  }
}
...
resource "azurerm_cdn_frontdoor_route" "app2" {
  ...
  forwarding_protocol    = "HttpsOnly"
  https_redirect_enabled = true
  patterns_to_match      = ["${var.app2_path}/*"]
  supported_protocols    = ["Http", "Https"]

  depends_on = [azurerm_cdn_frontdoor_origin.app2]
}
